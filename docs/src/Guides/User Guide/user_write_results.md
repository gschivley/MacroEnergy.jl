...
- [Curtailment Results](@ref): non-served demand (load curtailment) at nodes.
...
Export system-level curtailment results (non-served demand) using the [`write_curtailment`](@ref) function:

```julia
write_curtailment("curtailment.csv", system)
```

Curtailment results represent load that could not be served at nodes in the system. This can occur when there is insufficient generation capacity or transmission constraints prevent delivery of power to demand nodes.

Filter results by commodity using the `commodity` parameter:
...
write_curtailment("curtailment.csv", system, commodity="CO2*")
```
...
!!! note "Curtailment and Solution Algorithms"
    Curtailment results are automatically written when using all three solution algorithms (Monolithic, Myopic, and Benders decomposition) via the `write_outputs` function.
